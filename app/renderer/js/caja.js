// app/renderer/js/caja.js
/* Caja · Registro de Pedido — Ítems en dos filas + Lienzo + Preview */
(() => {
  (async () => {
    try {
  ...
  // pedidoId actual (puede venir por query o por contexto UI)
  ...
  async function init() {
    ...
    try {
      ...
      if (rawPrev) {
        ...
        // Si no tenemos currentPedidoId aún, intenta derivarlo de la migaja
  ...
  async function showPreviewForItem(index, card) {
    // 1) preferir migaja en localStorage (si justo venimos de guardar)
    const rawPrev = localStorage.getItem('rp:preview');
    ...
    const partId = data?.partidas?.[index]?.id;
  ...
  function wireEvents() {
    ...
    // === Finalizar: guarda, marca estado y limpia ===
    ...
    if (fin) {
      fin.addEventListener('click', async () => {
        try {
          ...
          resetForm();
          // limpia contexto para no arrastrarlo a un nuevo pedido
...
// === Navegar a Pedidos (si el rol lo permite ya está contemplado en main) ===
...
// === Logout: cierra sesión, limpia y envía a login ===
